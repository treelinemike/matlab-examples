...
figure;
set(gcf,'Position',[2.178000e+02 2.458000e+02 1.003200e+03 4.200000e+02]);
subplot(1,2,1);
hold on; grid on;
view([78,54]);
surf(X1,X2,J);
...
ylabel('\bfIntercept');
zlabel('\bfCost');
plot3(x_opt_analytic(1),x_opt_analytic(2),SSE_analytic,'.','MarkerSize',30,'Color',[0.8 0 0.8]);
...
allData(end,:) = [];
disp(allData);
plot3(allData(:,1),allData(:,2),allData(:,3),'.-','LineWidth',2,'Color',[0.8 0 0],'MarkerSize',20);
...
plot3(xc(1,:),xc(2,:),Jc,'-','LineWidth',3,'Color',[0.8 0 0.8]);
...
allData(end,:) = [];
disp(allData);
plot3(allData(:,1),allData(:,2),allData(:,3),'.-','LineWidth',2,'Color',[0 0.8 0],'MarkerSize',20);
...
optPosAlongConstraint = sqrt((allData(:,1)-xc(1,1)).^2 +(allData(:,2)-xc(2,1)).^2 )
plot(posAlongConstraint,Jc,'LineWidth',3,'Color',[0.8 0 0.8]);
plot(optPosAlongConstraint,allData(:,3),'.-','LineWidth',2,'MarkerSize',30,'Color',[0 0.8 0]);
